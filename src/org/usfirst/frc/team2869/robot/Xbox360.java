...
public class Xbox360 extends Joystick {
	    ...
	    public boolean getAnyButton() {
	    	return anySticks() || anyButtons();
	    }
	    private boolean anySticks() {
			return Math.abs(GetLeftX())>=.25 || Math.abs(GetLeftY())>=.25 ||
					Math.abs(GetRightX())>=.25 || Math.abs(GetRightY())>=.25 ||
					Math.abs(GetAllTriggers()) >= .25;
		}
	    
		private boolean anyButtons() {
			return GetStartValue()
					|| GetYValue() || GetXValue() || GetBValue() || GetAValue()
					|| GetRightBumperValue() || GetLeftBumperValue();
		}
		
		public void updatePresses() {
	    	startPressed = GetStartValue();
	    }
	    /**
	    ...
	    public double GetAllTriggers() {
	    	return (GetRawAxis(RIGHT_TRIGGER) - GetRawAxis(LEFT_TRIGGER));
	    }
	    ...
	    public boolean GetStartValueDown() {
	        return (GetRawButton(8)) && !startPressed;
	    }
